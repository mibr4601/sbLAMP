...
def parse_args():
    parser = argparse.ArgumentParser(
        prog="SingleNoCluster",
        usage="SingleNoCluster -in <ref_genome> [options]*",
        add_help=False,
    )
    ...
    parser.add_argument(
        ...
        default=0,
        metavar="<int>",
        help="Reference sequence: 0 picks most central sequence, otherwise Nth",
    )
    parser.add_argument(
        "-loop",
        dest="loop",
    ...
    if args.help:
        print("USAGE:")
        print("  SingleNoCluster -in <ref_genome> [options]*\n")
        print("ARGUMENTS:")
        print("  -in <ref_genome>")
        print("    Can be MSA or single sequence fasta")
        print("  -ref <int>")
        print("    0 picks the sequence closest to all the others;")
        print("    N picks the Nth sequence)")
        print("  -loop")
        print(
...
def _mean_hamming_rank(sequences):
    names = list(sequences.keys())
    n = len(names)
    arrs = [np.frombuffer(sequences[name].encode(), dtype=np.uint8) for name in names]
    best_idx = 0
    best_dist = None
    for i in range(n):
        d = sum(int(np.count_nonzero(arrs[i] != arrs[j])) for j in range(n) if j != i)
        if best_dist is None or d < best_dist:
            best_idx, best_dist = i, d
    return names[best_idx], best_idx


def select_reference(sequences, ref_num=0):
    names = list(sequences.keys())
    n = len(names)
    if n == 1:
        return names[0], 0
    ...
    lengths = {len(s) for s in sequences.values()}
    if len(lengths) == 1:
        return _mean_hamming_rank(sequences)
    ungapped = {name: sequences[name].replace("-", "") for name in names}
    best_name, _seq, _score = pick_most_similar(ungapped)
    return best_name, names.index(best_name)
...
if __name__ == "__main__":
    ...
    ref_name, ref_index = select_reference(sequences, args["ref"])
    ref_seq = sequences[ref_name].replace("-", "")
    print(
        f"Reference: {ref_name}  (#{ref_index + 1} of {len(sequences)}, {len(ref_seq)} bp)"
    )
    ...
    counts = candidate_primer(
        ref_seq, params, f_inner, f_outer, buf_loop, design=design
    )
    if f_loop:
    ...
    print(f"Total candidates: {counts[0]} inner, {counts[1]} outer, {counts[2]} loop")
